...
class CursoDetailViewTest(TestCase):
    def test_curso_detail_view(self):
        curso = Curso.objects.create(titulo='Curso 1', code='lalo', descripcion='This is a curso.')
        response = self.client.get(reverse('curso_detail', kwargs={'slug': 'lalo'}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'cursos/curso_detail.html')
        self.assertContains(response, curso.titulo)
        self.assertContains(response, curso.descripcion)
